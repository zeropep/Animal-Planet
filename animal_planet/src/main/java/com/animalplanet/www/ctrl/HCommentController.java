...
import com.animalplanet.www.service.HCommentService;
...
public class HCommentController {
	...
	@GetMapping(value = "/{hno}/{page}", produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<PagingHandler> CommentList(@PathVariable("hno") long hno, @PathVariable("page") int page) {
		PagingVO pgvo = new PagingVO(page, 10);
		return new ResponseEntity<PagingHandler>(csv.getList(hno, pgvo), HttpStatus.OK);
	}
