					...
					<div class="row">
							...
							<tbody>
								<c:forEach items="${list }" var="bvo">
									<tr class="border-bottom">
										...
										<c:choose>
											<c:when test="${bvo.getBTag() eq 'lifetalk'}"><a href="/freeboard/list?bCate=free&bTag=${bvo.getBTag() }">[일상 이야기]</a> </c:when>
											<c:when test="${bvo.getBTag() eq 'together'}"><a href="/freeboard/list?bCate=free&bTag=${bvo.getBTag() }">[함께 해요]</a></c:when>
											<c:when test="${bvo.getBTag() eq 'donation'}"><a href="/freeboard/list?bCate=free&bTag=${bvo.getBTag() }">[무료 나눔]</a></c:when>
										</c:choose>
										</th>
										<td class="small"><a
											href="/freeboard/detail?bno=${bvo.bno }">${bvo.title }</a></td>
						...
						<div class="pagination bg text-center w-100">
							<ul class="pagination justify-content-center">
								<c:if test="${pgn.prev }">
									<li class=""><a
										href="/freeboard/list?bCate=free&species=${pgn.pgvo.species }&bTag=${pgn.pgvo.getBTag() }&pageNo=${pgn.startPage - 1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}"
								...
								<c:forEach begin="${pgn.startPage }" end="${pgn.endPage }"
									...
									<li class=" ${pgn.pgvo.pageNo == i ? 'active':''}"
										...
										href="/freeboard/list?bCate=free&species=${pgn.pgvo.species }&bTag=${pgn.pgvo.getBTag() }&pageNo=${i }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">${i }</a>
									</li>
								</c:forEach>
								<c:if test="${pgn.next }">
									<li class=""><a class="page-link"
										href="/freeboard/list?bCate=free&species=${pgn.pgvo.species }&bTag=${pgn.pgvo.getBTag() }&pageNo=${pgn.endPage + 1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">Next</a>
									</li>
								</c:if>
							</ul>
							<div class="w-100 align-content-md-center text-center mt-10">
								<form action="/freeboard/list?bCate=free&species=${pgn.pgvo.species }" method="get">
