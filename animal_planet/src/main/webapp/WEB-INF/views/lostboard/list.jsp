					...
					<div class="row">
							...
							<tbody>
								<c:forEach items="${list }" var="bdto">
									<tr class="border-bottom">
										<th scope="row" class="small"><img src="/upload/${fn:replace(bdto.getFList().get(0).saveDir,'\\','/')}/${bdto.getFList().get(0).uuid }_th_${bdto.getFList().get(0).fileName}"></th>
										<td class="small">
										<c:choose>
											<c:when test="${bvo.getBTag() eq 'ing'}"><a href="/lostboard/list?bCate=lost&bTag=${bvo.getBTag() }">[찾고 있어요]</a> </c:when>
											<c:when test="${bvo.getBTag() eq 'done'}"><a href="/lostboard/list?bCate=lost&bTag=${bvo.getBTag() }">[찾았어요]</a></c:when>
										</c:choose>
										<a href="/lostboard/detail?bno=${bdto.getBvo().bno }">${bdto.getBvo().title }</a></td>
										<td class="small">${bdto.getBvo().nickName }</td>
										<td class="small">${bdto.getBvo().regAt }</td>
										<td class="small">${bdto.getBvo().readCount }</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						...
						<div class="pagination bg text-center w-100">
							<ul class="pagination justify-content-center">
								<c:if test="${pgn.prev }">
									<li class=""><a
										href="/lostboard/list?bCate=lost&species=${pgn.pgvo.species }&bTag=${pgn.pgvo.getBTag() }&pageNo=${pgn.startPage - 1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}"
								...
								<c:forEach begin="${pgn.startPage }" end="${pgn.endPage }"
									...
									<li class=" ${pgn.pgvo.pageNo == i ? 'active':''}"
										...
										href="/lostboard/list?bCate=lost&species=${pgn.pgvo.species }&bTag=${pgn.pgvo.getBTag() }&pageNo=${i }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">${i }</a>
									</li>
								</c:forEach>
								<c:if test="${pgn.next }">
									<li class=""><a class="page-link"
										href="/lostboard/list?bCate=lost&species=${pgn.pgvo.species }&bTag=${pgn.pgvo.getBTag() }&pageNo=${pgn.endPage + 1 }&qty=${pgn.pgvo.qty}&type=${pgn.pgvo.type}&keyword=${pgn.pgvo.keyword}">Next</a>
									</li>
								</c:if>
							</ul>
							<div class="w-100 align-content-md-center text-center mt-10">
								<form action="/lostboard/list?bCate=lost&species=${pgn.pgvo.species }" method="get">
