...
document.getElementById("cmtPostBtn").addEventListener("click", (e) => {
    ...
    if (data.content == null || data.content == "") {
        ...
        regComment(data).then(result => {
            ...
            getCommentList(data.npno);
        });
    }
});

async function spreadCommentFromServer(npno, page) {
    try {
        const resp = await fetch(`/pcomment/${npno}/${page}`);
...
function getCommentList(npno, page=1) {
    spreadCommentFromServer(npno, page).then(result => {
        // console.log(result);
...
document.addEventListener("click", (e) => {
    if (e.target.classList.contains("del")) {
        ...
        e.preventDefault();
        getCommentList(npnoVal, e.target.getAttribute("href"));
    }
});